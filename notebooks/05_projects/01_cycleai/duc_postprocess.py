...
def postprocess(imgs, labels, img_shape, result_shape):
    '''
    ...
    blended_img = Image.fromarray(cv.addWeighted(imgs[:, :, ::-1], 0.5, np.array(result_img), 0.5, 0))

    return confidence, result_img, blended_img, raw_labels


def score_model(input_img, raw_img, mod, result_shape):
    # get input and output dimensions
    result_height, result_width = result_shape
    _, _, img_height, img_width = input_img.shape
    ...
    mod.forward(batch([input_img]), is_train=False)
    ...
    # generate blended image
    # blended_img = Image.fromarray(cv.addWeighted(raw_img[:, :, ::-1], 0.5, np.array(result_img), 0.5, 0))
